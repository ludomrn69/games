/*
  ...
  transposition + approfondissement itératif, avec borne de nœuds pour ne jamais
  figer le navigateur. Le Puissance 4 étant résolu, une grande profondeur joue
  quasi parfaitement.
...
(function (root) {
  ...
  function bestColumn(board, me, opp, maxDepth) {
    ...
    var NODE_CAP = 220000, nodes = 0, aborted = false, TT = {};
    function search(bb, depth, alpha, beta, cur) {
      ...
      var a0 = alpha, key = bb + cur, tt = TT[key];
      if (tt && tt.d >= depth) {
      ...
      var other = cur === me ? opp : me, best = -1e9;
      var oc = ORD.filter(function (c) { return vc.indexOf(c) >= 0; });
      for (var i = 0; i < oc.length; i++) {
        var d = drop(bb, oc[i], cur), val;
      ...
      TT[key] = { d: depth, val: best, flag: best <= a0 ? -1 : best >= beta ? 1 : 0 };
      return best;
    }
